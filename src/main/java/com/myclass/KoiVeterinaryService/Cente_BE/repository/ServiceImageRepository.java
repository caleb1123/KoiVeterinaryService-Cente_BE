...
public interface ServiceImageRepository extends JpaRepository<ServiceImage, Integer> {
    @Query(value = "SELECT COUNT(*) AS image_count FROM service_image WHERE service_id = :id and status = 1 ", nativeQuery = true)
    Integer getImageCountByServiceid(int id);

    ServiceImage findByFileId(String fileId);

    @Query(value = "SELECT TOP 1 * FROM service_image WHERE service_id = :id AND status = 1 ORDER BY image_id ASC;", nativeQuery = true)
